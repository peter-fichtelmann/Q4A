...
class Config:
    ...
    COMPUTER_PLAYER_STEP_PROFILE_NUM_MOVES = 60
    ...
    FLAG_RUNNER_MAX_SPEED_REAL = 2.75  # m/s per real second
    ...
    FLAG_RUNNER_RANDOM_DIRECTION_FACTOR = 0.3  # how much randomness is added to the flag runner's direction
    FLAG_RUNNER_SEEKER_AVOIDANCE_FACTOR = 2.0  # how strongly the flag runner avoids seekers
    FLAG_RUNNER_BOUNDARY_AVOIDANCE_FACTOR = 10.0  # how strongly the flag runner avoids pitch boundaries
    FLAG_RUNNER_BOUNDARY_EPSILON = 1e-5  # prevents division by zero in boundary avoidance
    FLAG_RUNNER_INTERACTION_TIME_THRESHOLD = 3.0  # time in game time seconds that a seeker must be in contact with the flag runner to "catch" it
