...
class Config:
    ...
    PLAYER_THROW_VELOCITY_REAL = 6 # m/s per real second
    ...
    COMPUTER_PLAYER_KWARGS = {
        ...
        'score_interception_max_distance_per_step': 0.6, # for RuleBasedComputerPlayer, max distance to move per step when scoring
        'score_interception_max_dt_per_step': 0.4, # for RuleBasedComputerPlayer, max dt per step when scoring in s
        'scoring_threshold': 0.9, # for RuleBasedComputerPlayer, minimum interception score (chance of not being intercepted) to attempt a score
